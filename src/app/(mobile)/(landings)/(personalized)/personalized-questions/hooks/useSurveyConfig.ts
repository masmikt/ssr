...
import { PersonalizedQuestionsComponentsConfig } from '@/app/(mobile)/(landings)/(personalized)/personalized-questions/configs';
    ...
    const surveyConfig =
        useMemo<Record<PersonalizedQuestionsPages, IPageContentConfig> | null>(() => {
            ...
            return pagesList.reduce((result: any, page: PersonalizedQuestionsPages) => {
                if (page in PersonalizedQuestionsComponentsConfig) {
                    result[page] = PersonalizedQuestionsComponentsConfig[page];
                }
